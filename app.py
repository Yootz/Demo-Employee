...
conn = init_db()
if conn is None:
    raise RuntimeError("Failed to connect to the MySQL database.")

c = conn.cursor(named_tuple=True)
c.execute("CREATE DATABASE IF NOT EXISTS employee_db")
c.execute("USE employee_db")
c.execute(
    "CREATE TABLE IF NOT EXISTS roles (id INT AUTO_INCREMENT PRIMARY KEY, role_name VARCHAR(100), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
)
c.execute(
    "CREATE TABLE IF NOT EXISTS employees (id INT AUTO_INCREMENT PRIMARY KEY, first_name VARCHAR(100), last_name VARCHAR(100), pay DECIMAL(10, 2), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, roles_id INT, FOREIGN KEY (roles_id) REFERENCES roles(id))"
)

employee_pages = ["index", "add_employee", "edit_employee"]
role_pages = ["roles", "add_role", "edit_role"]


@app.route("/")
def index():
    c.execute(
        """SELECT e.id, e.first_name, e.last_name, e.pay, e.created_at, e.roles_id, r.role_name
                 FROM employees e
...
def add_employee():
    if request.method == "POST":
        ...
        c.execute(
            "INSERT INTO employee_db.employees (first_name, last_name, pay, roles_id) VALUES (%s, %s, %s, %s)",
            (first_name, last_name, pay, roles_id),
        )
        conn.commit()
...
def edit_employee(employee_id):
    if request.method == "POST":
        ...
        roles_id = request.form["roles_id"]
        c.execute(
            "UPDATE employee_db.employees SET first_name=%s, last_name=%s, pay=%s, roles_id=%s WHERE id=%s",
            (first_name, last_name, pay, roles_id, employee_id),
        )
        conn.commit()
        return redirect(url_for("index"))
...
def crud_employees():
    ...
    if request.method == "POST":
        ...
        pay = data.get("pay")
        c.execute(
            "INSERT INTO employee_db.employees (first_name, last_name, pay) VALUES (%s, %s, %s)",
    ...
    if request.method == "PUT":
        ...
        pay = data.get("pay")
        c.execute(
            "UPDATE employee_db.employees SET first_name=%s, last_name=%s, pay=%s WHERE id=%s",
            (first_name, last_name, pay, employee_id),
        )
        conn.commit()
